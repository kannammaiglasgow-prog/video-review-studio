 ...
 * it from Gemini first if it's running low. No external scraping at all —
 * Reddit's public API is blocked (403) and their official API's "Responsible
 * Builder Policy" restricts this exact AI-content-generation use case, so
 * premises are self-generated by Gemini instead (see generateIdeaBatch). */
async function fetchFreshIdea(genre: IdeaGenre): Promise<StoryIdea | undefined> {
  if (countUnusedIdeas(genre) < POOL_REFILL_THRESHOLD) {
    const batch = await generateIdeaBatch(POOL_REFILL_SIZE, genre);
    if (batch.length > 0) addIdeasToPool(batch, genre);
  }
  return getUnusedIdea(genre);
}
...
/** The FAST half of an automated run: pick a fresh premise, have Gemini write
 * the content from it (sized for the target format — a Short gets a short
 * script, not the long-form one truncated; devotional channels get a reverent
 * explanatory script instead of a drama-twist story), and create the project
 * row. Kept separate from the slow render half so idea-availability/Gemini
 * errors surface immediately to a caller awaiting this — e.g. the
 * manual-trigger API route — instead of being lost inside a fire-and-forget
 * background job. */
async function prepareAutoStoryIdea(channel: StoryChannel, format: StoryFormat = "long"): Promise<PreparedIdea | null> {
  const { genre, language } = CHANNEL_CONFIG[channel];
  const idea = await fetchFreshIdea(genre);
  if (!idea) return null;
