...
async function buildOptionImages(
  ...
  slides.forEach((slide, si) => {
    slide.answers.forEach((_answer, oi) => {
      // Always the English term: stock libraries and the image model only
      // understand English, whatever language the option is displayed in.
      prompts.push(optionImagePrompt(searchTerms[si]?.[oi] ?? "", category));
  ...
  await Promise.all(
    index.map(async (ref, i) => {
      const term = searchTerms[ref.slide]?.[ref.option] ?? "";
      if (!term) return;
      try {
...
async function renderFrames(b: QuizBrand, timeline: Timeline, framesDir: string, mascot: string | null): Promise<number> {
  ...
  const worker = async () => {
    for (;;) {
      ...
      const svg = renderFrameSvg(b, timeline, i / CANVAS.fps, mascot);
...
/** Full quiz Short for one channel: its branded template frames plus
 * synchronised voice/SFX/music in its own language. */
export async function renderQuizVideo(
  ...
  options: {
    ...
    category?: string;
    /** English search terms per slide/option for the illustrations. */
    searchTerms?: string[][];
    onPhase?: (phase: "rendering") => void;
  } = {},
  ...
  if (options.category && options.searchTerms) {
