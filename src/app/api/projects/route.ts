...
export async function POST(request: Request) {
  try {
    ...
    const insert = db.prepare(`INSERT INTO projects
      (youtube_url,start_time,end_time,stance,tone,persona,voice,aspect_ratio,duration,custom_instruction,status)
      VALUES (?,?,?,?,?,?,?,?,?,?,?)`);
    db.exec("BEGIN IMMEDIATE");
    try {
      const project = insert.run(input.url,input.startTime,input.endTime,input.stance,input.tone,input.persona,input.voice,input.format,input.duration,input.customInstruction || null,"queued");
