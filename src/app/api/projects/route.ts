...
export async function POST(request: Request) {
  try {
    ...
    const insert = db.prepare(`INSERT INTO projects
      (youtube_url,source_type,script_mode,transcript,start_time,end_time,stance,tone,persona,voice,tts_provider,aspect_ratio,duration,custom_instruction,status)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`);
    db.exec("BEGIN IMMEDIATE");
    try {
      const project = insert.run(input.url,input.sourceType,input.scriptMode,input.sourceType === "text" ? input.sourceText || null : null,input.startTime,input.endTime,input.stance,input.tone,input.persona,input.voice,input.ttsProvider,input.format,input.duration,input.customInstruction || null,"queued");
